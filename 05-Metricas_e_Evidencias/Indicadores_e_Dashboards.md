# Indicadores e Dashboards (sugestão)

[Voltar ao README](../README.md)

O material menciona dashboards em tempo real e painéis (estratégico e operacional). Abaixo, um conjunto de indicadores replicável.

## Indicadores de participação
- **Cidadãos inscritos**
- **Cidadãos participantes**
- Participação por **território/município**
- Participação por **oficina/eixo**

## Indicadores de produção de propostas
- **Total de propostas cadastradas**
- Propostas por território/eixo
- Temáticas abordadas por oficina/eixo
- Tempo médio para estruturar/validar proposta (se aplicável)

## Indicadores de priorização
- Total de votos/pontos
- Ranking (top 5 por oficina/território)
- Distribuição de pontuação

## Indicadores de qualidade/satisfação
- Total de avaliações
- Grau de satisfação (geral e por aspecto)
- Feedbacks qualitativos (principais temas)

## Publicação e transparência
- Link do **observatório** com propostas eleitas (e/ou lista completa).
- Base exportável (CSV) para auditoria social.

---

### Navegação
- [README](../README.md)
- Anterior: [Guia de Capacitação](../04-Operacao_e_Capacitacao/Guia_de_Capacitacao.md)
- Próximo: [Evidências públicas](./Evidencias_Publicas.md)
